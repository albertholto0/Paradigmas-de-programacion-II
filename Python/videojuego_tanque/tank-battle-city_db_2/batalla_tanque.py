...
def mostrar_historial(screen):
    try:
        ...
        cursor.execute("SELECT p.fecha, p.detalle_partida, v.ganador, v.puntuacion FROM partidas p JOIN victorias v ON p.id_partida = v.id_partida ORDER BY p.fecha DESC")
    ...
    encabezado_font = pygame.font.Font(None, 28)

    while True:
        screen.fill((30, 30, 30))  # Fondo negro

        # Encabezados de la tabla
        encabezado_fecha = encabezado_font.render("Fecha", True, (255, 255, 255))
        encabezado_detalle = encabezado_font.render("Descripción", True, (255, 255, 255))
        encabezado_ganador = encabezado_font.render("Ganador", True, (255, 255, 255))
        encabezado_puntuacion = encabezado_font.render("Puntuación", True, (255, 255, 255))

        screen.blit(encabezado_fecha, (50, 10))
        screen.blit(encabezado_detalle, (220, 10))
        screen.blit(encabezado_ganador, (500, 10))
        screen.blit(encabezado_puntuacion, (700, 10))

        y_offset = 50  # Offset inicial para el historial

        for entrada in historial:
            fecha, detalle, ganador, puntuacion = entrada
            texto_fecha = font.render(fecha.strftime("%Y-%m-%d %H:%M:%S"), True, (255, 255, 255))
            texto_detalle = font.render(detalle, True, (255, 255, 255))
            texto_ganador = font.render(ganador, True, (255, 255, 255))
            texto_puntuacion = font.render(str(puntuacion), True, (255, 255, 255))

            screen.blit(texto_fecha, (50, y_offset))
            screen.blit(texto_detalle, (220, y_offset))
            screen.blit(texto_ganador, (500, y_offset))
            screen.blit(texto_puntuacion, (700, y_offset))

            y_offset += 30  # Espaciado entre filas

        # Botón Salir
        boton_salir_rect = pygame.Rect(screen.get_width() // 2 - 100, screen.get_height() - 70, 200, 50)
        pygame.draw.rect(screen, (150, 50, 50), boton_salir_rect)
        texto_salir = font.render("Salir", True, (255, 255, 255))
        screen.blit(texto_salir, (boton_salir_rect.x + (boton_salir_rect.width - texto_salir.get_width()) // 2, boton_salir_rect.y + (boton_salir_rect.height - texto_salir.get_height()) // 2))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if boton_salir_rect.collidepoint(event.pos):
                    return  # Salir del historial y volver a la pantalla principal

def mostrar_pantalla_inicio(screen):
    """Muestra una pantalla inicial con botones funcionales 'Iniciar' e 'Historial'."""
    while True:
        ...
        # Mostrar botón "Historial"
        texto_historial = font.render("Historial", True, (255, 255, 255))
        boton_historial_rect = texto_historial.get_rect(center=(screen.get_width() // 2, boton_iniciar_rect.bottom + 60))
        pygame.draw.rect(screen, (50, 50, 50), boton_historial_rect.inflate(20, 10))  # Fondo del botón
        screen.blit(texto_historial, boton_historial_rect)  # Texto del botón
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if boton_iniciar_rect.collidepoint(event.pos):
                    return "iniciar"  # Salir de la pantalla de inicio y proceder a la solicitud de nombres de jugadores
                elif boton_historial_rect.collidepoint(event.pos):
                    return "historial"  # Mostrar el historial de partidas
        ...
        while running:
            ...
            if tanque1.vida <= 0 or tanque2.vida <= 0:
                if tanque1.vida <= 0 and tanque2.vida <= 0:
                    ...
                    tanque_ganador_puntuacion = float(tanque2.puntuacion if tanque1.vida <= 0 else tanque1.puntuacion)
